...
use ns_backend::app_state::{
    requested_virtual_slots, ServerContext, UsbControllerFamily, MAX_CLIENTS,
};
use ns_backend::controller_profiles::report_requests_pair;
use ns_backend::s2_rawgadget::{
    RawGadgetConfiguration, REENUMERATION_DISCONNECT_INTERVAL,
};
...
use ns_backend::web_server::WebServer;
    ...
    while context.is_running() {
        ...
        loop {
            match socket.recv_from(&mut packet_buffer) {
                Ok((size, address)) => {
                    ...
                    if let Some(control) = inspect_control_datagram(
                        &context,
                        &key,
                        bytes,
                        requester_slot,
        ...
        if Instant::now() >= next_write {
            ...
            if let Some(service) = s2_service.as_mut() {
                let outcome = service.tick(&context, now_us, S2ReportContext::default());
                match outcome {
...
fn send_rejection(
    ...
    let flags = reason_flag | if sleeping {
