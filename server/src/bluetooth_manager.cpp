...
constexpr auto        RECONNECT_COOLDOWN       = std::chrono::seconds(5);
...
std::string path_to_mac(const std::string& path) {
    constexpr std::string_view marker = "/dev_";
...
std::string trim_copy(std::string_view s) {
    ...
    auto end   = std::ranges::find_if_not(s | std::views::reverse,
                                          [](unsigned char c) { return std::isspace(c); }).base();
    return start < end ? std::string(start, end) : "";
}

// ---------------------------------------------------------------------------
// BlueZ main.conf fast-connect policy
// ---------------------------------------------------------------------------

bool ensure_ini_key(std::vector<std::string>& lines,
                    std::string_view section,
                    std::string_view key,
                    std::string_view value) {
    const std::string sec_header = std::format("[{}]", section);
    const std::string wanted     = std::format("{} = {}", key, value);
    ...
    for (auto it = sec_it + 1; it != end_it; ++it) {
        ...
        if (t.starts_with(key) &&
            (t.size() == key.size() || std::isspace(t[key.size()]) || t[key.size()] == '=')) {
...
void configure_bluez_reconnect_policy(bool verbose) {
    if (geteuid() != 0) return;
    (void)run_cmd("mkdir -p /etc/bluetooth >/dev/null 2>&1");
...
// ===========================================================================
// BlueZ D-Bus controller manager (requires sdbus-c++)
// ===========================================================================
...
struct DeviceInfo {
    ...
    bool looks_fresh_from_discovery() const {
        return !name.empty() || !alias.empty() || !icon.empty()
            || has_hid_uuid || klass != 0 || appearance != 0;
    }
};
...
// ---------------------------------------------------------------------------
// Agent registration (auto-accept all pairings — no PIN needed for gamepads)
// ---------------------------------------------------------------------------
...
bool BluezManager::ensure_adapter() {
    ...
    auto it   = std::ranges::find_if(objs, [](const auto& pair) {
        return pair.second.contains(ADAPTER_IFACE);
    });
    if (it == objs.end()) {
...
bool BluezManager::adapter_set_bool(const char* prop, bool value) {
    ...
    try {
        ...
        if (g_ctx.verbose)
            std::println(stderr, "[bt] adapter {}={} failed: {}", prop, value, e.getMessage());
        return false;
    }
}
...
bool BluezManager::set_trusted(const DeviceInfo& dev, bool trusted) {
    ...
    try {
        ...
        if (g_ctx.verbose)
            std::println(stderr, "[bt] failed to trust {}: {}", dev.display_name(), e.getMessage());
        return false;
    }
}
...
bool BluezManager::call_device_method(const std::string& path,
    ...
    try {
        ...
        if (g_ctx.verbose && e.getName() != "org.bluez.Error.NotReady") {
            std::println(stderr, "[bt] method {} failed on {}: {}",
                         method, path_to_mac(path), e.getMessage());
        }
        return false;
    }
}
...
bool BluezManager::pair_device(const DeviceInfo& dev) {
    ...
    return call_device_method(dev.path, "Pair", DBUS_PAIR_TIMEOUT,
                              "org.bluez.Error.AlreadyExists");
}

bool BluezManager::connect_device_once(const DeviceInfo& dev) {
    if (!connection || dev.path.empty() || dev.connected) return true;
    return call_device_method(dev.path, "Connect", DBUS_CONNECT_TIMEOUT,
...
// ---------------------------------------------------------------------------
// Shared pairing helper (used by both tick() and run_pairing_wizard())
// ---------------------------------------------------------------------------

bool BluezManager::try_pair_one(const DeviceInfo& dev) {
    (void)stop_discovery();
...
void BluezManager::run() {
    ...
    while (g_manager_running.load(std::memory_order_relaxed)) {
        ...
        for (int waited = 0;
             waited < wait_ms && g_manager_running.load(std::memory_order_relaxed);
             waited += 25) {
...
// ---------------------------------------------------------------------------
// Interactive pairing wizard (--pair)
// ---------------------------------------------------------------------------
...
void BluezManager::run_pairing_wizard() {
    ...
    for (const auto& dev : list_devices()
        ...
        try {
            ...
            proxy(adapter_path)->callMethod("RemoveDevice")
                                .onInterface(ADAPTER_IFACE)
                                .withTimeout(DBUS_FAST_TIMEOUT)
                                .withArguments(sdbus::ObjectPath{dev.path})
                                .storeResultsTo();
        } catch (const sdbus::Error& e) {
            std::println(stderr, "[bt] failed to remove device {}: {}",
                         dev.display_name(), e.getMessage());
        }
    }
    ...
    while (!wizard_done.load(std::memory_order_relaxed) && paired_count < 4) {
        for (const DeviceInfo& dev : list_devices()
                                   | std::views::filter([](const auto& d) {
                                         return !d.blocked && d.is_controller_like();
                                     })) {
            if (dev.paired || dev.trusted) {
                if (!dev.trusted) (void)set_trusted(dev, true);
...
// ===========================================================================
// Stub for builds without BlueZ D-Bus
// ===========================================================================
