...
int cli_main(const std::vector<std::string>& original_args) {
    ...
    if (args.size() < 2) {
        std::println(stderr, "Usage: {} --cli <RASPBERRY_PI_IP[:PORT]> [-m MACRO] [-k [single|override]]", args[0]);
        return 1;
    }
    ...
    // CLI mode always emulates a Pro Controller; the controller type saved by
    // the GUI must not leak into CLI sessions. Audio follows the saved GUI
    // feature toggles, which only activate for the Pro type anyway.
    g_controllerType.store(ns::CONTROLLER_TYPE_PRO);
